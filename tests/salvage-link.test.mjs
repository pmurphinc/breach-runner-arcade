...
test("gameplay uses one pickup resolver and consumes a linked round after its first hit", () => {
  ...
  assert.match(game, /const resolvePlayerPickup =/);
  assert.match(game, /const collected = resolvePlayerPickup\(game, pickup\);[\s\S]{0,220}bullet\.life = 0/);
  assert.match(game, /if \(pupCollected\(pickup, player\)\) \{\s*resolvePlayerPickup\(game, pickup\);/);
  assert.match(game, /game\.stock\.length >= STOCK_LIMIT[\s\S]{0,180}return false/);

...
export type FireShot = { kind: "projectile" | "flame"; weaponId: string; evolutionId: string | null; instanceId: string; hardpointIndex: number; salvoIndex: number; origin: Point; angle: number; damage: number; speed: number; radius: number; life: number; penetrations: number; explosionRadius: number; range: number; coneDegrees: number };
